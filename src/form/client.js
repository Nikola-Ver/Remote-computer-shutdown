...
function showMessage(msg) {
  ...
  if (messageBox.classList.contains("off")) messageBox.classList.remove("off");
...
function changeVol(vol) {
  document.documentElement.style.setProperty("--vol-value", String(vol) + "%");
...
function mouseMoveVolume(e) {
  if (flagMouseDownVolume) {
    console.log("yes 1");
    shift = Math.ceil(((startPosVolume - (e.screenX ? e.screenX : e.changedTouches[0].screenX)) / document.body.clientWidth) * 2 * 100);
    let vol = currentVol - shift < 0 ? 0 : currentVol - shift > 100 ? 100 : currentVol - shift;
    changeVol(vol);
  }
}
...
function mouseUpVolume(e) {
  if (flagMouseDownVolume) {
    ...
    currentVol = currentVol - shift < 0 ? 0 : currentVol - shift > 100 ? 100 : currentVol - shift;
...
function mouseMove(e) {
  if (flagMouseDown && !flagBlockDisplayVolume) {
    shift = Math.ceil((((e.screenY ? e.screenY : e.changedTouches[0].screenY) - startPos) / document.body.clientHeight) * 100);
    let vol = currentVol - shift < 0 ? 0 : currentVol - shift > 100 ? 100 : currentVol - shift;
    changeVol(vol);
  }
}
...
function mouseMoveMediaSwitch(e) {
  if (flagDownMediaSwitch) {
    delta = ((e.screenY ? e.screenY : e.changedTouches[0].screenY) - startPosMediaSwitch) / 120 + prevDelta;
    if (delta > 0 && delta < 1) {
      document.documentElement.style.setProperty("--media-height", String(delta));
    } else if (delta < 0) {
      delta = 0;
